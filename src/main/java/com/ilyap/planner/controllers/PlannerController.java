package com.ilyap.planner.controllers;

import com.ilyap.planner.PlannerMain;
import com.ilyap.planner.TasksModel;
import com.ilyap.planner.utils.InfoUtil;
import com.ilyap.planner.utils.TaskUtil;
...
public class PlannerController implements Initializable {
    ...
    private void createCalendarModel(LocalDate date, double rectangleHeight, double rectangleWidth, StackPane stackPane) throws IOException {
        ...
        List<TasksModel> noCheckedTasks = InfoUtil.getListByDate(date.format(InfoUtil.FORMATTER)).stream()
    ...
    private List<LocalDate> getCalendarModelMonth() throws IOException {
        return InfoUtil.getList().stream()
                ...
                .map(dateStr -> LocalDate.parse(dateStr, InfoUtil.FORMATTER))
                .toList();
    }
